...
> 原文：<https://www.sitepoint.com/overcome-cache-conundrums/>
...
为了使这种方法有效，样式表中使用的图像必须位于样式表所在位置的子文件夹中(例如，`/cimg/`而不仅仅img/`)。这无论如何都是有意义的，因为样式表使用的资源在逻辑上是与样式表本身组合在一起的。
...
因为样式表中的图像是相对于样式表的路径的，所以您使用的图像的路径现在是`/css.12img/foo-background.png`！这个 URL 将被`RewriteRule`重写，就像 CSS 文件本身的 URL 一样。
