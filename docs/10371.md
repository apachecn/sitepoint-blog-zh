...
> 原文：<https://www.sitepoint.com/the-anatomy-of-a-javascript-design-pattern/>

这是[我最喜欢的 JavaScript 设计模式](https://www.sitepoint.com/my-favorite-javascript-design-pattern/)的后续文章，有望回答它提出的一些问题，并对该模式的一些特性进行更深入的讨论。

那些将我描述的模式与一个 [JavaScript 模块模式](http://www.adequatelygood.com/2010/3/JavaScript-Module-Pattern-In-Depth)相比较的人是准确的，因为它看起来确实是一种非常相似的构造。克里斯蒂安·海尔曼的[揭示模块模式](http://www.wait-till-i.com/2007/08/22/again-with-the-module-pattern-reveal-something-to-the-world/)增加了一个漂亮的额外特性，通过提供更多的灵活性来公开方法。(感谢链接！)

但是所有复杂的设计模式都抛出了同一个基本问题——首先使用它有具体的好处吗？如果不了解好处，很容易认为这只是一个毫无意义的冗长和过度设计的解决方案，或者它只是在遵循某种时尚。
...
```
var
OLDER_WEBKIT = /applewebkit/([0-4]|[5][0-2])/i.test(navigator.userAgent),
...
我最后想看的是使用<dfn>特权值</dfn>的好处。它们实际上只是私有值，可以使用公共方法重新定义。
