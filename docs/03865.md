...
> 原文:[https://www . site point . com/deploying-PHP-apps-digital ocean-DP loy-io/](https://www.sitepoint.com/deploying-php-apps-digitalocean-dploy-io/)
...
![Deploybot logo](../Images/10256324d6de10d428c2879eedcf8efb.png)
...
![Connecting a repository](../Images/9a3aa74234cd28fd25b6d34b93526cb0.png)

然后，我们添加一个环境。由于我们正在创建一个生产环境，因此我们将保留默认值—手动部署。

![Configuring an environment](../Images/71993539d11f18f93d7b13d9e4653979.png)

在部署下，我们选择 Web 应用程序下的数字海洋。如果你对什么是什么感兴趣的话，读一读这些问号。

![Choosing a destination](../Images/15b7b2ec1df453d00f6e67342124682e.png)

在下一个设置屏幕上，我们需要填写一些附加字段。最重要的是:

*   我们需要将应用程序路径设置为`/usr/share/nginx/spsearch`。
    ![Application path changed](../Images/5035fe64d60da587e4cfee09e1b5ccd9.png)
*   我们需要提供一个静态文件，因为正在讨论的应用程序使用一个`token.php`文件来提供一个 Diffbot 令牌，但是这个文件不受版本控制，以防止泄漏和窃取。
    ![Static file added](../Images/d897db48c285ef5d55f96a7fbecd0e94.png)
*   一旦部署完成，我们需要告诉 DeployBot 启动`composer install`。我们使用预启动脚本这样做，是因为我们希望在新版本与当前版本进行符号链接时，应用程序已准备就绪，而不是在 Composer 完成安装时遭受启动后停机。这可以通过将`composer install`放入`pre-launch`窗口来完成:
    ![Pre-deploy command added](../Images/fc14f7932e5761cea9b38bdb9bb52460.png)
...
![Ready screen](../Images/f4f0a5de1aba3cff0cbf7dea1d8e4a51.png)
...
![Deployment pre-launch screen with note field](../Images/1e93818929ddf8b120fb3ed37d5ff074.png)

一旦点击“开始部署”，DeployBot 就会进入正题。

![Deploying...](../Images/cbeeaef2fa0742a2c053f2d2eb402545.png)

根据应用程序的大小和它可能包含的二进制文件，此过程可能需要一段时间，但一旦完成，您应该能够注意到文件都在您想要的位置:

![Deployed app](../Images/0347f8c9917184ff7a06948fa2071aba.png)

让我们看看它在浏览器中是否有效。

![Working site](../Images/a607a6a52516aa722d7c39af387be86d.png)
