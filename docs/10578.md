...
> 原文:[https://www.sitepoint.com/s3-php-part-2/](https://www.sitepoint.com/s3-php-part-2/)
...
| [![1](../Images/2bd701092fce9649f6264fffb1477ca8.png)T2】](#co-getbucketobjects-1) | 我们的函数接受三个参数:一个`AmazonS3`对象、一个 S3 桶和一个默认为空字符串的前缀值。 |
| [![2](../Images/10981f9191fce48990425d9eb70b7088.png)T2】](#co-getbucketobjects-2) | 我们使用一个`do … while`循环，这样循环体总是至少运行一次。 |
| [![3](../Images/804bdb4d00412807d025c7abb911a625.png)T2】](#co-getbucketobjects-3) | 每次调用`list_objects`，我都会传入一个名为`$next`的值。第一次通过循环，`$next`是一个空字符串，`list_objects`从桶的开头(按字母顺序排列)开始。在后续的循环迭代中，`$next`被设置为前一次迭代返回的最后一个键。这告诉 S3 按照前一次迭代的最后一个键的字母顺序开始检索键。 |
| [![4](../Images/8d8734ec57bc389554ff03bed3ee0715.png)T2】](#co-getbucketobjects-4) | 如果`list_objects`调用失败，函数返回 null 。 |
| [![5](../Images/bcadf753cb2d4323243ddeb12438e475.png)T2】](#co-getbucketobjects-5) | 我们从返回给我们的`list_objects`调用的响应体中检索`Contents`数组，然后遍历存储在`$objects`数组中的每个值。这个数组最终将是我们的返回值。 |
| [![6](../Images/98418cb78c55784733a7faeeead0a527.png)T2】](#co-getbucketobjects-6) | 调用`list_objects`返回的数据包含一个名为`IsTruncated`的元素。如果这个值是字符串`"true"`，列表是不完整的，有更多的对象要被发现。这个条件也用于控制环路。 |
| [![7](../Images/abd57a378e0a5d1708118af94ef845e5.png)T2】](#co-getbucketobjects-7) | 如果列表不完整，我们设置`$next`值准备开始下一次迭代。 |
| [![8](../Images/eb701ba0e985908c66197ece278961c9.png)T2】](#co-getbucketobjects-8) | 当循环终止时，返回`$objects`数组。 |
...
| [![1](../Images/2bd701092fce9649f6264fffb1477ca8.png)T2】](#co-list-objects-page-1) | 这段代码检查是否提供了`bucket`参数。如果是，那么它被用作`$bucket`的值。否则，使用默认值`BOOK_BUCKET`常量。 |
| [![2](../Images/10981f9191fce48990425d9eb70b7088.png)T2】](#co-list-objects-page-2) | 这里我们调用自定义的`getBucketObjects`函数，该函数获取给定桶中的对象列表，并将它们存储在`$objects`数组中。 |
| [![3](../Images/804bdb4d00412807d025c7abb911a625.png)T2】](#co-list-objects-page-3) | 下一步是迭代数组并处理每个数组。 |
| [![4](../Images/8d8734ec57bc389554ff03bed3ee0715.png)T2】](#co-list-objects-page-4) | 我们在`$fileList`数组中为每个对象存储三个值:对象的 URL、键(我们存储为`name`)和大小(转换为整数并格式化为数字)。 |
| [![5](../Images/bcadf753cb2d4323243ddeb12438e475.png)T2】](#co-list-objects-page-5) | 我们包含了 HTML 模板来输出`$fileList`数组中的值。 |
...
![Listing objects in an S3 bucket](../Images/fb5ecc0e76f7b212d3fd0f9966f501e9.png)
