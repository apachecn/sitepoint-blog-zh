...
> 原文:[https://www.sitepoint.com/build-faq-asp-ms-access/](https://www.sitepoint.com/build-faq-asp-ms-access/)
...
![Filling the FAQ table with sample data](../Images/d3509bf090ae8372f020798254b928ae.png)注意，对于新条目,`Views`列应该总是被设置为零，以表示它们还没有被查看。为了简化这一点，请确保将该列设置为 Required，默认值为 0。如果您发现您需要的问题超过了`Text`规定的 255 个字符的限制(`Memo`最多允许 65，535 个字符)，您还可以将`Question`列从`Text`更改为`Memo`数据类型。
...
数据库提供者对于 ADO 就像设备驱动程序对于操作系统一样。这是一款允许 ADO 以标准方式与给定类型的数据库进行通信的软件。ADO 附带了用于 Access、SQL Server、Oracle 和 ODBC 数据库服务器等的内置提供程序。T3】
...
![The Completed FAQ Page](../Images/85ae37d2fb31d1e12d12a2744ca70c9f.png)打开 MS Access，进行一些更改，上传新的`faq.mdb`文件，刷新浏览器，查看所做的更改！现在任何可以使用 Microsoft Access 的人都可以更新您的 FAQ 页面了！现在，如果你认为这很棒，看看我们接下来做什么…
...
![A Single FAQ Answer](../Images/561f283c455a2ba8bf45455d5eb0c747.png)太好了！现在剩下的就是跟踪每个问题被查看的次数(还记得我们的`FAQ`表中尚未使用的`Views`列吗？)，并使用该值以合理的顺序向访问者展示 FAQ 列表。
