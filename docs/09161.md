...
> 原文:[https://www.sitepoint.com/php-namespaces/](https://www.sitepoint.com/php-namespaces/)
...
1.  **全限定名**T5】全限定名就像文件系统中的绝对路径。它是标识符的全名；因此，对于引用哪个标识符没有歧义。完全限定名以反斜杠开头。例如，`My\Foo\Bar`在名称空间`My\Foo`中寻找类`Bar`。
2.  **限定名**T5】限定名就像文件系统中的相对路径。它只包含名称的一部分，并相对于当前命名空间进行解析。例如，名称空间`MyBaz`中的`Foo\myFunction()`解析为`\My\Baz\Foo\myFunction()`。
3.  **非限定名**
