...
> 原文：<https://www.sitepoint.com/getting-started-with-rails-3-part-2/>

欢迎来到我们两部分教程的第二部分。[上次](https://www.sitepoint.com/getting-started-with-rails-3-part-1/)，我们在 Rails 上安装了 Ruby，生成了一个缩短 URL 的应用程序，并为它创建了模型。
...
![fig2](img/50588b93386c433ddede0b2122728aa1.png)
...
*   当用户提交新表单时，我们应该尝试创建一个新的 URL。如果它有效并被保存，我们将设置一条消息告诉用户他们被缩短的链接的 URL，然后我们将再次重定向回被缩短的页面。
...
这是一个非常基本的 HTML5 页面，除了在 body 标签中呈现 action 视图之外什么也不做。在这个页面上，我们要改变的唯一方面(为了在将来扩展应用程序时使其可重用)，是当我们缩短一个链接时，我们会在页面顶部获得一个指向它的链接。因此，在`<body>`开始标记的下方和带有`yield`的行的上方，添加以下内容:
...
![fig3](img/9584c73bfbbb876c836c33dda9d6c346.png)

如果您点击**缩短我的网址**，而没有在**您的网址**字段中输入任何网址，您将收到以下错误页面。

![fig4](img/fa74eee7f024e9d160680403a27673a0.png)

如果您在**您的网址**字段中输入一个有效的网址，并点击**缩短我的网址**(例如`http://google.com/`，您将看到下图。

![fig5](img/68b78648e932091c29a3b54b625e32b6.png)
...
当然，我们构建的应用程序并非没有缺陷。例如，当我们验证一个 URL 实际上是给定的，我们不验证它*是*一个 URL(并且我们盲目地重定向到那个值)；因此，这不是您实际部署的那种代码。同样，工作流本身也非常初级。
