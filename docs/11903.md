...
> 原文:[https://www.sitepoint.com/php-amp-mysql-1-installation/](https://www.sitepoint.com/php-amp-mysql-1-installation/)
...
*   从[WAP server 网站](http://www.wampserver.com/en/)下载最新版本。下载文件后(截至本文撰写时，WampServer 2.0g 大小约为 16MB)，双击它启动安装程序，如下图所示。
    ...
    ![The WampServer installer](../Images/2f4a1d8c02eb98060716eb3cff7c38fb.png)

*   安装程序将提示您安装 WampServer 的位置。默认的 c:wamp 如图 1.2 所示，“默认的安装目录是一个好的选择”对于大多数目的来说是一个理想的选择，但是如果你对它的安装位置有强烈的感觉，请随意指定你的首选位置。

    ![The default installation directory is a good choice](../Images/2c7d36115e6c9637dcf80a1d594afc9a.png)
...
*   安装结束时，WampServer 会要求您选择默认浏览器。这是当您使用附带的系统托盘图标工具启动浏览器时，它将启动的 web 浏览器。如果你安装了 Firefox，它会询问你是否愿意将它作为你的默认浏览器。如果您回答“否”,或者安装了不同的浏览器，它将要求您为您要使用的浏览器选择可执行文件。如图 1.3，“默认选择 Internet Explorer 就可以了”，它为你选择了 Internet Explorer(explorer.exe)，这就可以了。如果您使用的是 Safari 或 Opera 等替代浏览器，您可以浏览找到。exe 文件，如果你想的话。
    ...
    ![The default choice of Internet Explorer is fine](../Images/8c92e76e7a651eb96e6a35ec721c5489.png)
...
*   当安装 WampServer 时，它启动 Apache HTTP Server 的内置副本，这是一种用于 PHP 开发的流行 web 服务器。此时，Windows 可能会显示一个安全警报，如图 1.4 所示，“这个安全警报告诉您 Apache 正在执行它的任务”，因为 web 服务器试图开始监听来自外界的浏览器请求。
    ...
    ![This security alert tells you Apache is doing its job](../Images/abdf12df0d7212b43e379f057ae77ec0.png)
...
![Fill in your Internet Service Provider's SMTP server address if you know it](../Images/e689f51bdfff178683b3a65c1a267b5f.png)

安装完成后，您就可以启动 WampServer 了。一个图标将出现在您的 Windows 系统托盘中。点击它可以看到下面显示的 WampServer 菜单。

![The WampServer menu](../Images/dd1720e57d73389e1819afdcc04887cf.png)
...
![The home page provided by WampServer confirms Apache, PHP, and MySQL are installed](../Images/991b154e5de0709dfffee63d325f23d6.png)
...
![Finding the right link can be tricky -- here it is!](../Images/c08fed628320e46c7ae325f73b0d9c47.png)
...
![The best available version -- accept no substitutes!](../Images/237cd27872fbdd3fde6f865e84432d8d.png)

进入下载页面后，向下滚动找到各种可用版本的链接。您需要的是不带加密的 Win32 二进制文件，如下所示。

![This is the one you need](../Images/06c9794771357b019f49583261ee8b60.png)
...
![The green light means Apache is up and running](../Images/2eb838c617c40deb1f094024173670f1.png)

否则，您将需要手动启动 Apache，如下所示，然后才能使用它。

![Choose Start to fire up Apache manually](../Images/2e9910503b8f972a8d5de1fcc15509c8.png)
...
![You can take my word for it!](../Images/942c632614c3b75c3422508d4fa56a60.png)
...
![Make filename extensions visible for all files](../Images/7fc3ff5f4b7abbbcf2345bac857bfb82.png)
...
![The PHP version number indicates Apache is configured to support PHP](../Images/7cd6a9bc0bf04bbea026da5aa57e0170.png)
...
![The MAMP package](../Images/62232f0fdcfb52f8889df50fd7407b52.png)
...
![The MAMP window](../Images/c4a4cb944c28d9d2eb3330fa78409892.png)

当内置的 Apache 和 MySQL 服务器启动时，这两个状态指示器将从红色变为绿色。接下来，MAMP 将打开您的默认 web 浏览器并加载 MAMP 欢迎页面，如下所示。

![The MAMP welcome page confirms Apache, PHP, and MySQL are up and running](../Images/a08b07d7120b0a877ac96f2e248a555c.png)
...
![The 32-bit version of MySQL for Intel processors will work on most current Macs](../Images/6e68ceeae8f36570766428c92774ae78.png)
...
![The MySQL Mac OS X package contains lots of goodies](../Images/0db4a9c4092f869485b848e452930e05.png)
...
![The MySQL System Preferences pane](../Images/cf8f1c83607bcff1ac29f88bcbb1a227.png)
...
![Enable Web Sharing in Mac OS X](../Images/98e5946349a354a4009cdeb7246c8f87.png)

*   退出“系统偏好设置”。*   打开你的浏览器，在地址栏输入`http://localhost`,然后回车。您的浏览器应该显示如下所示的标准 Apache 欢迎消息。

![The standard Apache welcome page](../Images/2541deaa626a8efb1bf8e1a66e88ec35.png)
...
![Click the lock to make changes to these settings](../Images/269aee65f02a2702667162ff2d5308cd.png)

要使该文件可编辑，请将 Privilege 列中的值更改为 every one Read & Write，如下所示。

![Set the permissions for everyone to Read & Write](../Images/7d4854dedc61b7541ac9446429013d50.png)
...
![Finding the right link can be tricky - here it is!](../Images/2befb20c22ef8686b5cc1b494d0b6bb6.png)
...
![This is the one you need](../Images/eda0fa73811190620e091b040601cb33.png)
...
![You can take my word for it!](../Images/b5ac3daf9b9fb8cea7f27ba00cfbb8d1.png)
...
![See your first PHP script in action!](../Images/27916ccaa85ab16df44d6dcd7f61ed28.png)
