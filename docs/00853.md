...
> 原文:[https://www . site point . com/optimization-auditing-a-deep-dive-into-chromes-dev-console/](https://www.sitepoint.com/optimization-auditing-a-deep-dive-into-chromes-dev-console/)
...
![Snippets tool](../Images/5a2aa90067e65e1bae75abf32f3f1f18.png)
...
![Accessing and docking the console](../Images/6b2440f8503b18065697df5ba5af2716.png)
...
![Clear history](../Images/ff0c6ed87b54851753ca31b8fd5dd6a1.png)

您可以在页面刷新或更改之间保留日志(通过启用保留日志复选框)，直到您清除控制台或关闭选项卡。

![Preserving the log](../Images/78e5776d4845585aba7f3dc80c6d8dc6.png)

您可以将控制台中的历史保存为文本文件，方法是在控制台中右键单击并选择*另存为…* ，然后选择日志文件的位置。

![Save history](../Images/debf93f793c1389540bbaaedc6067807.png)
...
![Store as global variable](../Images/352f480b1a34e07556f55ee0fce417b9.png)
...
![passing strings](../Images/f6961b088ac7799e3812d27b2127f49a.png)
...
![Adding format specifiers](../Images/4ef48526b7f326d28dcde3068a8dc301.png)
...
![Asserting with console.assert](../Images/a0ccad64baf3d1d46e863f05af3697fe.png)

### 使用 console.count()计算执行次数

`console.count(label)`输出它在同一行和同一标签上被调用了多少次。

![console.count](../Images/6a07534aae41024b15358157e99994a1.png)
...
![console.count](../Images/f93ba96475b4e42b0b8e27271f6f8c30.png)
...
![Adding flags to the Console output](../Images/a5dbc52349403a734e2a62eabfa4c564.png)

#### 过滤控制台日志

使用前面的方法，您可以按严重性级别过滤控制台输出。

![Severity levels](../Images/e8aaca8b33eba514e3cd2cab8d7b457e.png)

您也可以通过文本过滤输出。

![Filter log by text](../Images/4ef2ddeb9a26c57fe90502a5257f8833.png)
...
![Grouping messages into groups](../Images/52cbec87e067ec6aca52f5308c48c5a3.png)

我们还可以嵌套组:

![Nesting groups](../Images/01cc1b0b5f94c6edc80b3f8faba3a000.png)
...
![console.table](../Images/919edd09263685a9ca5e36d3e446293f.png)
...
![console.dir](../Images/455eac250f0d2a8d3110a521b939401b.png)
...
![cpu profiles](../Images/9a4803cf4eedb5a664dbe15e9c9cbe2f.png)

您可以使用 JavaScript Profiler 面板来查找您创建的配置文件，这允许您查看每次运行该函数所花费的时间。

![How much time the function takes to execute](../Images/298965fd5faff9ad4fb6afc7bb5b6d06.png)
...
![Measuring execution time](../Images/056e689a74953ea8740efbbf06d27c71.png)
...
![Function tracing](../Images/3afb8ed82cb73a52a78af32a57703dcf.png)
...
![Monitor events](../Images/3b69cf0eb4512d92b8f478ad3089ee88.png)
...
![Reference selected element](../Images/99d2984a9c8419456e47df51c999f63f.png)
...
![Recording a profile for runtime performance](../Images/2d6f9ae6e43263f7c99692d80a589e7a.png)
...
![Measuring load time performance](../Images/2e50a5a51733fa447821d15cd55f9115.png)
...
![Incognito mode](../Images/9eb4d3d5a7e7ca34f86c2130c0af6e5c.png)
...
![Choosing network settings](../Images/416fec573f032a6af550232c6d65045a.png)

您还可以添加自定义条件，以更好地针对用户群的条件:

![Setting a custom network profile](../Images/fbea0f31b742bbc2933f807e4a7c6570.png)

### 模拟移动 CPU

CPU 节流允许您模拟移动设备缓慢的 CPU 能力，以便您可以准确地测试您的网页在移动设备中的执行情况。

![CPU throttling](../Images/421b64262f27d6f788597c19954c0c34.png)
...
![Taking screenshots while profiling](../Images/5a7e23892262c184bc12477436f4dab2.png)

### 分析性能报告

在优化应用程序的性能之前，您需要对其进行分析，原因有二:第一，测量性能有多差，第二，确定可以进行优化的地方。生成性能报告后，您可以了解不同图表的含义以及如何使用其他子工具。

![A performance report](../Images/c3cedff29821ce4f6e1b1d4fc9bece47.png)
...
![zooming](../Images/9108184eb6d08b734282359ef5f7c600.png)
...
![FPS meter](../Images/e11e56db5586c26361b8531404efc340.png)

### FPS 图表

FPS 图表显示分析过程中每秒的帧速率。

![FPS chart](../Images/2db73da497f0fa37fa2805c082fc6d82.png)

正如谷歌、*指出的那样，FPS 图表中的红线表明帧速率太低，以至于损害了用户体验。*

下面是优化代码的另一个截图，没有红色条:

![FPS chart, no red](../Images/36af8f9ad8fcccaf917f428d39b018c0.png)
...
![Clicking on a frame](../Images/2c10a62155c33a847597401f23e09903.png)

### CPU 图表

CPU 图表显示了分析期间 CPU 的活动。它位于 FPS 图表的下方。

![CPU chart](../Images/bce8ac8d9b2050b21a152a23af980d69.png)

颜色对应于摘要选项卡中的不同活动。如果你看到 CPU 图表充满了颜色，这意味着 CPU 长时间处于最大使用状态。这是你做一些性能优化的信号。

#### 网络图和网络部分

![The net chart](../Images/7a6e2a1924ca35857eb8053573f33790.png)

网络图显示了分析期间的网络请求。它对于显示加载请求的网络部分的加载时间分析特别有用。

![Loading requests](../Images/887dc3502c6a88addfffc6d1404f943c.png)

当您将鼠标移到某个请求上时，它会显示该特定请求的加载时间。

当您单击特定请求时，所有其他子工具都会更新，以包括仅在请求期间发生的操作。

![Network section](../Images/558787c24d667200d73d32e5c485b173.png)

不同的颜色代表不同的资源类型——Java script、CSS、图像等。加载时间较长的资源具有较长的条形。每个条较亮的部分代表第一个字节或 **TTFB** 的**时间。较暗的部分代表**传输时间**。**

### “摘要”选项卡

![Summary Tab](../Images/77f5332ec02cc53226f2cb68fabac7be.png)
...
![Showing info about a selected function](../Images/305304387f76d6173ce968c69dbe7a52.png)
...
![Tabs beside Summary](../Images/eff0d8c0a73a9f80678d7aa1089dfdde.png)
...
![Flame graphs](../Images/df88370cfcea721cac71c53b150fbf6c.png)
...
![Flame map](../Images/6a709ad948d5c11aeb2e83ab1bc7fa86.png)
...
![The Bottom-Up map](../Images/301ef7a857f82b4c41bdecde8122742a.png)

*[图片来源](https://developers.google.com/web/updates/2016/12/devtools-javascript-cpu-profile-migration)*

旧的 CPU 档案器中的树(自上而下)视图可以被调用树选项卡所取代。

![Tree map](../Images/0ddfd1280b060225a0bf2ae48da38310.png)
...
![Audits report](../Images/b327e8115efc1811532e4c90cd7e0e93.png)
